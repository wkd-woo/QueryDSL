...
public class QuerydslBasicTest {
    ...
    void theta_join() {
        ...
        // 모든 회원, 모든 팀을 다 가져온 다음, username과 name으로 조인
        // DB가 성능 최적화 하긴 함.
        List<Member> result = queryFactory
                .select(member)
    ...
    void join_on_no_relation() {
        ...
        List<Tuple> result = queryFactory
                .select(member, team)
                .from(member) // from 절에 여러 엔티티를 선택해서 세타 조인
    ...
    void fetchJoinUse() {
        ...
        boolean loaded = emf.getPersistenceUnitUtil().isLoaded(findMember.getTeam());
        assertThat(loaded).as("페치 적용 미적용").isTrue();
    }
     ...
     */
    @Test
    void findDtoByFieldDifferentAlias() {
        QMember memberSub = new QMember("memberSub");
    ...
    void findDtoByQueryProjection() {
        ...
        // 다만 DTO에 QueryDsl 어노테이션을 유지해야 하는 점과 DTO 까지 Q파일을 생성해야 하는 단점이 있다.
        List<MemberDto> result = queryFactory
